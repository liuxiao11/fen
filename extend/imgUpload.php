 ...
 */
class imageUpload {
    const ROOT_PATH = ROOT_PATH . 'public' . DS . 'uploads/';
...
//    }
    //兼容PHP4
//    public function image($save_name,$save_dir) {
////        $this->__construct ( $save_name );
//    }
    public function stream2Image($save_name,$save_dir) {

//        set_error_handler ( $this->error_handler () );
        //设置保存图片名称，若未设置，则随机产生一个唯一文件名
        $save_name = $save_name ? $save_name : md5 ( mt_rand (), uniqid () );

        //设置保存图片路径，若未设置，则使用年/月/日格式进行目录存储
        $save_dir = $save_dir ? self::ROOT_PATH .$save_dir : self::ROOT_PATH .date ( 'Y/m/d' );
        ...
        if (!empty($data)) {
            ...
            if (@$fp = fopen ( $save_fullpath, 'w+' )) {
                fwrite ( $fp, $data );
                fclose ( $fp );
                $baseurl = "http://" . $_SERVER ["SERVER_NAME"] . ":" . $_SERVER ["SERVER_PORT"] . dirname ( $_SERVER ["SCRIPT_NAME"] ) . '/' . $save_name;
